...
int lookupSparse (std::string string, int dense){
    ...
    for (i = 0; i < string.length() && s_haschild[pos] == 1; i++){
        found = false;
    ...
    if (i == string.length()) {
        return (found ? 0 : 1); //percorremos toda a string e chegamos ao fim
    }
    else { //esgotamos a árvore mas não a string, possível falso positivo
        if (found){
            return 1;
        } else return 0;
    }
